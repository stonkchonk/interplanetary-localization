...
if __name__ == "__main__":
    ...
    star_cam.turn_precisely('y', 45)
    image = star_cam.take_screenshot('horizontal')
    image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    cv2.imwrite(f"horizontal.png", image)
    star_cam.turn_precisely('z', 45)
    image = star_cam.take_screenshot('angle45')
    image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    cv2.imwrite(f"angle45.png", image)
    star_cam.turn_precisely('z', 70)
    image = star_cam.take_screenshot('angle115')
    image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    cv2.imwrite(f"angle115.png", image)
